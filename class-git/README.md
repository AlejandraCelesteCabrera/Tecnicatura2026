...
```
## >  history #Veremos la historia completa de los comandos que hemos utilizado  <br>
```sh
...
### >  IMPORTANTE: SI YA ESTA CREADO EL REPOSITORIO NO SE DEBE VOLVER A PONER ESTE COMANDO <br>
```sh
...
code .
```
## > Modificamos el archivo historia.txt colocando lo siguiente: Bienvenido     mi nombre es Ariel (coloca tu nombre) <br>
```sh
ctrl + s
git status
git add .
...
git commit 
```
## Sin agregar -m veremos que pasa, se abre el editor de texto vim
```sh
## >Agregar mensaje y salir con <br>
...
```
## >Agregamos otra línea de mensaje en historia.txt desde VSC: estoy estudiando programación<br>
```sh
ctrl + s
git add .
git commit
```
## >Se abre un editor de código basado en línea de comandos, editor de texto como VSC llamado vim<br>
```sh
...
```
## >Copiamos un hash mas antiguo y otro reciente, ingresamos el siguiente comando<br>
```sh
...
```sh
cd tecnicatura 
#Vamos a hacer pruebas, es por esto que creamos una carpeta nueva
cd practicas
 #Entramos en la carpeta
touch reset_file.txt
 #Agregar información y hacer uno a dos commits
...
git show #Nos muestra ahora que esta en el HEAD -> master, cabecera aquí es donde esta apuntando, es decir el último commit esta pegado a dos amas distintas, aunque todavía estemos en master
q #Para salir
...
```sh
­Para crear commits rápidamente, fusionando git add y git commit -m "", usamos git commit -am "".­> Para generar nuevas ramas, hay que posicionarse sobre la rama que se desea copiar y utilizar el comando git branch .
